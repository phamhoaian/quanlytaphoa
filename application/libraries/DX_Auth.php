	...
	*/
		...
		if ($login)
		{
			...
			if ($query = $this->ci->users->get_login($login) AND $query->num_rows() == 1)
			{
				...
				if ( ! $row->newpass_key)
				{
					...
					$data['reset_password_uri'] = site_url($this->ci->config->item('DX_reset_password_uri').rawurlencode($row->username)."/{$data['key']}");
	...
	{		
		...
		if ($query = $this->ci->user_temp->activate_user($username, $key) AND $query->num_rows() > 0)
		{
			...
			// Create user
			if ($user_id = $this->ci->users->create_user($row))
			{
				// Trigger event
				$this->ci->dx_auth_event->user_activated($user_id);	
